...
def make_pred_multilabel(model, directory, save_dir, rijeka=False):
    """
    ...
    for i, sample in enumerate(dataloader):
        ...
        '''
        resnet_untillast = nn.Sequential(*list(model.children())[:-1])
        for param in resnet_untillast.parameters():
            param.requires_grad = False
        acts = resnet_untillast(inputs).cpu().data.numpy()
        '''
...
def make_pred_singlelabel(model, directory, save_dir, rijeka=True):
    BATCH_SIZE = 1
    model.train(False)
    dataset = MRIDataset(directory=directory, 
                        mode='valid', rijeka=rijeka,
    ...
    for i, sample in enumerate(dataloader):
        inputs, labels = sample['buffers'], sample['labels']
        inputs, labels = Variable(inputs.cuda()), Variable(labels.cuda())
        true_labels = labels.cpu().data.numpy() # shape: [4, 1, ]
        batch_size = len(true_labels)
        outputs = model(inputs)
        probs = outputs.cpu().data.numpy()
        scanlist = dataset.scanlists
